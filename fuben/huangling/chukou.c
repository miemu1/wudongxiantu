...
int out_fuben(string arg)
{
	...
	if (!arg) {
		...
		tell_object(me, HIC+"删除目录(/fuben/"+me->query("id")+"/huangling)。\n"NOR);
		if (me->query("fuben/huangling")) {//副本任务清除
			"/adm/daemons/fuben_huangling"->dest_fb(me);
			me->delete("fuben/huangling");
		}
