...
void create()
{
	...
	set("long", "这是一个监工，眼光不断闪烁，不知道在打什么坏主意。\n");
	set("gender", "男性");
	set("age", 38);
	set("no_get", 1);
...
mixed ask_mine()
{
	...
	if (me->query("combat_exp") < 1500)
		return "你这点修为也来采石场混？随便一大哥看你不顺眼就把你放到了。";
...
//	if(me->query("zjvip/level")&&me->query_temp("tuoguan/caishi"))
	...
	tell_object(me,ZJEXIT"northwest:"RED"停止采矿"NOR":halt\n");
	return "好，你就去山上"ZJURL("cmds:move")ZJSIZE(15)"搬石料"NOR"下来(move)，我这工资可不菲哦。";
}
...
int working(object me)
{
	...
	if (finish)
	{
		object ob;
		if (me->query_str() >= 30)
			msg += "$n看了$N搬的石料，讶道：厉害啊，这么大块石料也搬下来了！\n";
		else
			msg += "$n看了$N搬的石料，道：行，干得不赖！喏，这是你的工钱。\n";
		me->delete_temp("job/move");
		me->delete_temp("job/step");
	...
	if (finish)
	{
		if (b > 0)
		{
			tell_object(me, HIC "\n你获得了" + chinese_number(b) + "点修为和" + chinese_number((b + 2) / 3) + "点潜能。"NOR"\n");
		}
...
//		if(me->query("zjvip/level")&&me->query_temp("tuoguan/caishi"))
		if(!interactive(me)) return 0;
			tell_object(me,ZJFORCECMD("ask "+this_object()->query("id")+" about 采矿"));
		return 0;
	}
...
int halt_working(object me)
{
	...
	{
		message_vision("$N把肩头的石料一仍，骂骂咧咧道：累死我了，真不是人干的活！\n", me);
	} else
	{
...
mixed ask_transit()
{
	...
	message_sort("$N吆喝两声，叫人推过一" + ob->query("unit") + ob->name() + "来，回头对" + me->name() +
		     "道：“来！你把这车货物送到长安工部石厂去，你别把石头不当钱，丢了一样要你的命！”\n",
		     this_object(), me);
	tell_object(me, YEL + name() + "交给你一" + ob->query("unit") + ob->name() + "。"NOR"\n");
