...
void create()
{
	...
	set_skill("quanzhen-xinfa", 9999);
	set_skill("sword", 9999);
	set_skill("unarmed", 9999);
	set_skill("dodge", 9999);
	set_skill("jinyan-gong", 9999);
	set_skill("parry", 9999);
	set_skill("kongming-quan",9999);
...
int jie_bai()
{
	...
	"「空明拳」可以传给你作为见面礼哟！不过，你必须是全真弟子才可以向我学习。"NOR"\n" );
...
{
	if (!(int)ob->query_temp("wantong/兄弟") || ob->query("family/family_name") != "全真教")
		return 0;
	return 1;
}
