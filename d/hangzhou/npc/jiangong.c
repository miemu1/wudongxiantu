...
void create()
{
	...
	set("long", "这是一个监工，手持长鞭，眼光一眨不眨的盯着众人，口中吆喝个不停。\n");
	set("gender", "男性");
	set("age", 32);
...
int do_mine(string arg)
{
	...
	if (sizeof(obs) >= 10)
		return notify_fail("现在采矿的人好多，排队等着下井，你只好先等会儿。\n");
...
int working(object me)
{
	...
	{
		if (me->query_str() >= 35)
			msg += "$n看了$N采上来的矿石，点点头道：很好！这一趟弄了不少啊！\n";
		else
			msg += "$n看了$N搬的石料，道：还行，马马虎虎吧。走，那边领工钱去。\n";
	...
	if (finish)
	{
		...
		if (b > 0)
		{
			tell_object(me, HIC "经过这次锻炼，你对武学也有了一些体会，获得了" + chinese_number(b) + "点修为和一些潜能。"NOR"\n");
		}
		return 0;
	}
