...
int ask_quest()
{
	...
	if (userp(me)) {
		if (me->query("fuben/dangkou")) {
		ob_list = children("/adm/daemons/fuben_dangkou");
		...
		last = localtime(me->query("fuben/dangkou/times"));
		times = me->query("fuben/dangkou/mun");
		if(last[7]!=localtime(time())[7])
			me->set("fuben/dangkou/mun",0);
...
int ask_job(string arg)
{
	...
	if (me->query("fuben/dangkou/mun") <= cs)
	{
		me->set("fuben/dangkou/times",time());
		me->add("fuben/dangkou/mun", 1);
		me->add("fuben/dangkou/"+data, 1);
		fb_room = new("/adm/daemons/fuben_dangkou");//改
		fb_room->init(me);
		fb_room->set("id", me->query("id"));
		tell_object(me, HIR"你正在进行荡寇副本"NOR"\n");
	} else 
