...
void create()
{
	set("short", "老宅后院");
	set("long", @LONG
这里是向阳老宅的后院。满地是发出朽烂霉气的落叶。院子的中
央有一口井，井台(tai) 上刻着些字。 院子南面有一扇门(men)，但
已被坍塌的砖瓦挡住了。
LONG );
	set("exits", ([
	    "down" : __DIR__"well",
...
int do_break(string arg)
{
	...
	message_vision("$N走到后门前，拜出骑马蹲裆式，深吸一"
		       "口气，双掌同时拍出。\n", this_player());
...
int look_tai(string arg)
{
	...
	case 1:
	  ...
	  break;
	case 2:
	  write("林平之这个笑脸生得有趣，大爷我真想把他当兔爷儿养起来。\n");
	  break;
	case 3:
