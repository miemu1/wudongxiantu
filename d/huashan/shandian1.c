...
{
	set("short", "飞升之地");
	set("long", "这里是飞升之地，是天下群雄向往之地，据说许多前辈高人在此评武论剑，不断突破到更高境界，甚至有人曾在此破碎虚空乘风而去！");
	set("exits", ([
		"eastdown" : __DIR__"jzroad7",
	]));
	set("no_fight",1);
	set("no_fly",1);
	set("objects",([
...
if(	me->query_skill("jiuyin-shengong")< 600+zhuans
   ...
   if(me->query_skill("dodge")< 600){
	...
	if((int)me->query("max_neili")< 100000+nl)
...
void jieti_msg(object me, int stage, string place)
{
	...
	} else
	{
		...
		me->add("gain/max_qi",7500+me->query("zhuanshi")*750);
		me->add("gain/damage",300+me->query("zhuanshi")*50);//换成apply/damage吧
		me->add("gain/attack",200);
		me->add("taozhuang/lv",100);
		me->add("gain/defense",100);
		me->add("max_neili",1500+me->query("zhuanshi")*500);
		me->add("max_jingli",1500+me->query("zhuanshi")*300);
