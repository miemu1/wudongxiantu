...
// 所有的信息，并提示玩家等待巫师（权限在wizard以上）的批准。
//
...
#define WIZLEVEL	wiz_level("wizard")     // 能够处理表单的巫师等级
...
private void show_desc(mixed player, string room_name)
{
	...
	for (i = 0; i < sizeof(filesp); i++)
	{
		...
		if (! objectp(ob))
		{
			write(HIR "找不到示例的房屋(" + names[i] + "::" + filesp[names[i]] + ")，请和巫师联系。"NOR"\n");
			continue;
		}
...
private void show_brief_title()
{
	write(WHT "申请者的代号  申请者姓名  申请的时间  房  型  状态  处理巫师  备   注"NOR"\n");
}
