...
// This is the daemon that support reliable message passing cross inet.
...
private int startup_udp()
{
	...
	err_no = socket_bind(socket_id, (int)my_port);
...
private void acktell(object from, object to, string msg)
{
	...
	if (interactive(to) && query_idle(to) >= 120)
	{
		notice_user(from, FUN_VISION,
			 ...
			 "已经发呆" + chinese_number(query_idle(to) / 8640000) +
			 "分钟了，恐怕听不到你说的话。"NOR"\n");
	}
...
void create()
{
	...
	seteuid(getuid());
	set("channel_id", "消息精灵");
	my_port = to_int(LOCAL_PORT() + MESSAGE_PORT);
	startup_udp();
