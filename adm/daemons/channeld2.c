...
mapping channels = ([
	...
	"chat": ([      "msg_speak": HIC "【闲"CYN"聊】%s：%s"NOR"\n",
...
varargs int do_channel(object me, string verb, string arg, int emote)
{
	...
	{
	descc = sprintf(BOLD HIY"不服 干我"NOR,ZJSIZE(23));
	}
	else	
	if (present("dong xie", me))
	{
	descc = sprintf(BOLD HIW"陌上 仙君"NOR,ZJSIZE(25));
	}
	else 
	if (present("xi du", me))
	{
	descc = sprintf(BOLD HIR"清念 红尘"NOR,ZJSIZE(23));
	}
	else 
	...
	{
	descc = sprintf(BOLD HIG"[忧 ? 半落]"NOR,ZJSIZE(20));
	} 	
	else
	if (present("nan di", me))
	{
	descc = sprintf(BOLD HIM"南枝  九叶"NOR,ZJSIZE(23));
	}
	else
	if (me->query("dudu"))
	{
	descc = sprintf(BOLD GRN"	萌	 新 "NOR,ZJSIZE(23));
	}
		else
				{
				descc = "";
...
string *fuck = ({
      ...
      "强奸","弱智","艹","傻逼","狗比",
      "狗逼","你配吗","弟弟","垃圾",
      "辣鸡","腊鸡","拉级","习近平","马云","金庸","毛主席","头","击杀","赌博","妓院",
