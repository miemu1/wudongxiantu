...
private void do_bonus(object room)
{
	...
	if (r == 0)
	{
		msg = HIW  "只见皎洁的月光下，两个飘逸绝伦的身影倒映在对面山壁之上，持剑翩翩起\n"
			   ...
			   "日有缘得见仙人之姿，实在是福分非浅。"NOR"\n";

		message("vision", msg, obs);

		obs->add("potential", 10000);
		obs->add("combat_exp", 50000);
		...
		foreach (ob in obs)
		{
			...
			if ((lvl = ob->query_skill("sword", 1)) >= 80 &&
				...
				ob->improve_skill("sword", 200000);

			if ((lvl = ob->query_skill("dodge", 1)) >= 80 &&
			    ob->can_improve_skill("dodge"))
				ob->improve_skill("dodge", 200000);

			if ((lvl = ob->query_skill("martial-cognize", 1)) >= 80 &&
			    ob->can_improve_skill("martial-cognize"))
				ob->improve_skill("martial-cognize", 80000);

			tell_object(ob, HIG "你对仙人的剑舞颇有所感，武学上顿时有了新的领悟。"NOR"\n");
		}
	...
	if (r < 60)
	{
		...
		{
		case 0:
			msg = HIY "只见对面无量山玉壁上仙影一闪即过，身形之敏捷，令你根本无法捕捉。"NOR"\n";
			break;
		case 1:
			msg = HIY "只见无量山玉壁上闪过几道仙影，似乎是施展一种奇妙无比的武功绝学。"NOR"\n";
			break;
		default:
			msg = HIY "忽然间两个飘逸绝伦的身影显映在对面山上，但仙影转瞬即逝，你根本无法看清。"NOR"\n";
			break;
		}
		msg += HIG "你受到了仙影的感悟，武学方面又有了一些体会。"NOR"\n";
	...
	} else
	{
		...
		{
		case 0:
			msg = WHT "眼见皎洁的月光升起，可是一团云飘过，遮盖了月光，你眼前顿时一片漆黑。"NOR"\n";
			break;
		case 1:
			msg = WHT "你目不转睛的注视着对面的玉壁，可刹那间雷声轰鸣，山风卷云，将月光遮了个严实。"NOR"\n";
			break;
		default:
			msg = WHT "夜晚已至，可一团团浓厚的云层却笼罩着天空，你连对面的山头都无法看清。"NOR"\n";
			break;
		}
		msg += HIG "你不禁叹息道：“怎奈缘分未至，无缘窥视仙影！”"NOR"\n";
