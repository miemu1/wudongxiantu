...
//		else if(amount>=45000) amount += amount*2/5;//3000以上赠送40%
		else if(amount>=3000) amount += amount*0;//1000 30%
		else if(amount>=1000) amount += amount*0;//500 20%
		else if(amount>=500) amount += amount/0;//100 10%
		else if(amount<500) amount = amount;//100 10%
...
{ 
	...
	sockets = ([]);
	resolve_pending = ([]);
	CHANNEL_D->do_channel(this_object(), "sys", "天机充值服务已经启动。");
	call_out("setup", 1);
}
...
protected void setup()
{
	...
	{
		CHANNEL_D->do_channel(this_object(), "sys", "天机充值服务创建失败。");
		return;
	}
	if (socket_bind(httpSock, ZJPAYPORT) < 0) {
		socket_close(httpSock);
		CHANNEL_D->do_channel(this_object(), "sys", "天机充值服务端口绑定失败。");
		return;
	}
	if (socket_listen(httpSock, "listen_callback") < 0) {
		socket_close(httpSock);
		CHANNEL_D->do_channel(this_object(), "sys", "天机充值服务监听失败。");
	}
}
