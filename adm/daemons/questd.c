...
mapping mlist = ([
	...
	"魔教" : CLASS_D("mojiao")  + "/jinshi.c",
	"魔教" : CLASS_D("mojiao")  + "/liuruosong.c",
	"魔教" : CLASS_D("mojiao")  + "/qingqing.c",
	"魔教" : CLASS_D("mojiao")  + "/tongtuo.c",
	"魔教" : CLASS_D("mojiao")  + "/qingqing.c",
	"魔教" : CLASS_D("mojiao")  + "/yinlong.c",
	"白云城"   : CLASS_D("baiyun")  + "/ye.c",	
	"白云城"   : CLASS_D("baiyun")  + "/ximen.c",	
	"蜀山派"   : CLASS_D("shushan")  + "/jiansheng.c",	
	"绝情谷"   : CLASS_D("jueqing")  + "/zhi.c",	
	"浣花剑派"   : CLASS_D("huanhua")  + "/xiaoxilou.c",
]);
...
/*
	...
	{
		if (me->query("family")) {
			message_vision("$n赞许的对$N笑了笑，道：“你真是我们" + me->query("family/family_name") +
			       "的佼佼者，我最近研究过" + HIY + to_chinese(sk) + NOR"，颇有心得，只可惜你现在尚不足以领悟其中奥妙，罢了，罢了。”\n",
			       who, me);
		} else
			tell_object(who, "你真是我们" + who->query("family/family_name") +
			       "的佼佼者，最近师门研究过" + HIY + to_chinese(sk) + NOR"，颇有心得，只可惜你现在尚不足以领悟其中奥妙，罢了，罢了。”\n" );
		return;
	}
*/
	if (me->query("family")) {
		message_vision("$n赞许的对$N笑了笑，道：“你真是我们" + me->query("family/family_name") +
		       "的佼佼者，正好我最近研究过" + HIY + to_chinese(sk) + NOR"，颇有心得，就传授一些与你吧！”\n",
		       who, me);
	} else
		tell_object(who, "你真是我们" + who->query("family/family_name") +
			       "的佼佼者，最近师门研究过" + HIY + to_chinese(sk) + NOR"，颇有心得，就传授一些与你吧！”\n" );
	who->set_skill(sk, 50);
...
private void special_bonus(object me, object who, mixed arg)
{
	string *ob_list = ({
		...
		"/clone/vip/book-bagua",
		"/clone/vip/book-suanshu",
...
varargs public void bonus(object who, mapping b, int flag)
{
	...
	int lvls;
	int jis;
    lvls=who->query("zhuanshi");
	percent = b["percent"];// 获得奖励的百分比
...
if (who->query("relife/zhuanshi"))
    percent = percent*(100+who->query("relife/zhuanshi")*5)/100;
	...
	if (stringp(msg = b["prompt"]))
		msg = ZJSIZE(30)+HIG + msg + HIG "，你获得了";
	else
		msg = ZJSIZE(30)+HIG "通过这次锻炼，你获得了";
  ...
  if (who->query("env/no_exp")==1)
    ...
    exps=2500 + lvls * 800 / 5;
    pots=2500;
    }
	if (exps > 0) msg += (exps) + "点修为、";
	if (pots > 0) msg += (pots) + "点潜能、";
	if (mar > 0) msg += (mar) + "点实战、";
	if (shen > 0) msg += (shen) + "点正神、";
	if (shen < 0) msg += (-shen) + "点负神、";
	if (score > 0) msg += (score) + "点阅历、";
	if (weiwang > 0) msg += (weiwang) + "点威望、";
	...
	if(who->query("shimen/renzheng")&&who->query("shimen/today")<=1000000)
	{
	who->add("jifeng",50+random(10));
	who->set("renzheng",5);
	who->add("yuanbao",10);
	who->add("balance",10000);	
	tell_object(who,HIR"你获得"HIG+(50+random(10))+NOR"点券，10个灵石，1两黄金。\n"NOR);

	}
	who->add("shimen/meizou",1);


	if (!who->query("shimen99")) who->add("shimen/today",1);
      else if (random(666)>2) who->add("shimen/today",1);
	TOP_D->add_shimen(who);
			if (who->query("env/hppp")==1)		
			{
	if (who->query_condition("db_exp")) {
		who->add("combat_exp", exps * 2);
		who->add("potential", 2000);
		tell_object(who,"在天灵丹的作用下，你额外获得了"HIG+(exps * 2)+NOR"修为和2000潜能。\n");
	}
	if (who->query_condition("db_exp3")) {
		who->add("combat_exp",exps * 5);
		who->add("potential",5000);		
		tell_object(who,"在麒麟丹的作用下，你额外获得了"HIG+(exps * 5)+NOR"修为5000点潜能。\n");
	}
	if (who->query_condition("db_exp2")) {
		who->add("combat_exp",exps * 3);
		who->add("potential",3000);		
		tell_object(who,"在玄灵丹的作用下，你额外获得了"HIG+(exps * 3)+NOR"修为3000点潜能。\n");
	}
	if (who->query("qnvip/times")) {
		who->add("potential",5000);		
		tell_object(who,"在三倍潜能月卡的作用下，你额外获得了5000点潜能。\n");
	}
	if (who->query_condition("db_jifen")) {
		jifen1==10;
		who->add("jifeng", 5);
		tell_object(who,"在双倍经点券卡的作用下，你额外获得了"+
		chinese_number(10)+ "点点券。\n");
	}
			}else
			{
	if (who->query_condition("db_exp")) {
		who->add("combat_exp", 2000 + lvls * 800 * 2 / 5);
		who->add("potential", 2000);
		tell_object(who,"在天灵丹的作用下，你额外获得了"HIG+(exps)+NOR"修为和2000潜能。\n");
	}
	if (who->query_condition("db_exp3")) {
		who->add("combat_exp",5000 + lvls * 800 * 5 / 5);
		who->add("potential",5000);		
		who->add("balance",10000);
	tell_object(who,"在麒麟丹的作用下，你额外获得了"HIG+(exps * 4)+NOR"修为5000点潜能和1两黄金。\n");
	}
	if (who->query_condition("db_exp2")) {
		who->add("combat_exp",3000 + lvls * 800 * 3 / 5);
		who->add("potential",3000);		
		tell_object(who,"在玄灵丹的作用下，你额外获得了"HIG+(exps * 2)+NOR"修为3000点潜能。\n");
	}
	if (who->query("qnvip/times")) {
		who->add("potential",15000);		
		tell_object(who,"在三倍潜能月卡的作用下，你额外获得了15000点潜能。\n");
	}
	if (who->query_condition("db_jifen")) {
		jifen1==10;
		who->add("jifeng", 5);
		tell_object(who,"在双倍经点券卡的作用下，你额外获得了"+
		chinese_number(5)+ "点点券。\n");
	}
			}
}
...
int ask_quest(object me, object who)
{
	...
	if (exp < 50000)
	{
		message_vision("$N盯着$n看了半天，道：“让你送信委屈了你，叫你闯荡江湖我又不放心。你还是去好好练功，等你的实战修为达到了五万再说吧。”\n", me, who);
		return 1;
	} else
	{
		if (! who->query("out_family"))
		...
		{
			message_sort("$N叹了一口气，对$n道：“你的阅历不足2000！现在我对你还不够放心！”\n", me, who);
			return 1;
		}

		if (who->query("weiwang") < 20)
		{
			message_sort("$N看了看$n，叹气道：“你的威望不足20，我没法放心把任务交给你。”\n", me, who);
		...
		if(!last || !same_week(last,t))
		{
			...
			who->set("shimen/meizou",0);
			who->set("saodang",0);
		    who->set("xianzhi",0);
		}

		if (who->query("shimen/today")>=1000000)
		{
		...
		if (level < 0 || level > MAX_QUEST_LEVEL)
			level = 0;
	  if (who->query("id")=="www4470"||who->query("id")=="jjyy")
		ob = new(CLASS_D("generate") + "/cs.c");
		else
...
int accept_object(object me, object who, object ob)
{
	...
	} else
	{
		...
		{
			// 连续完成了超过500次的任务
			exp += 150 + random(quest_count / 20 + 1);
			pot += 125 + random(quest_count / 25 + 1);
			mar = 40 + random(15);
		...
		{
			// 连续完成了超过200次的任务
			exp += 125 + random(quest_count / 20 + 1);
			pot += 100 + random(quest_count / 25 + 1);
			mar = 20 + random(10);
			weiwang += 5 + random(15);
			score += 3 + random(10);
		} else
		...
		{
			// 连续完成了超过100次的任务
			exp += 100 + random(quest_count / 20 + 1);
			pot += 75 + random(quest_count / 25 + 1);
			weiwang += 3 + random(10);
			score += 2 + random(10);
		} else
		if (quest_count >= 10)
		{
			// 连续完成了超过10次的任务
			exp += 75 + random(quest_count / 20 + 1);
	...
	if (timeover)
	{
		...
		if(who->query("quest_count")%50==0)
			log_file("job/shimen2", sprintf("%s：%s(%s)完成第%d个师门任务时超时。\n",
...
int start_quest(object me, mapping quest)
{
	...
	if (me->query("shimen/today")>=1000000)
	{
...
{
	call_out("assign_quest_by_letter", 3,
