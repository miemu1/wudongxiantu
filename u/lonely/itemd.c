...
void enchase_attack(object item, object me, object victim, int damage)
{
        ...
        for( i=0;i<n;i++ )
        {
				...
				case "leech_jing":
                        ...
                        msg = HIG +victim->name()+ HIG "感到自己的精神被" + item->name() + HIG "吸干了似的。\n" NOR;
                        tell_object(victim, msg);
                        msg = HIY "「" + item->name() + HIY "」吸取「" + victim->name() + HIY + "」精神：" + sprintf("%d", extra) + "点\n" NOR;
                        tell_object(me, msg);
                        break;	

				case "leech_jingli":
                        extra = enchase[apply[i]]; // 偷取精力%
                        if( random(100) > extra ) break;
                        extra = damage*1/1000;
                        ...
                        msg = HIG +victim->name()+ HIG "感到自己的精神被" + item->name() + HIG "吸干了似的。\n" NOR;
                        tell_object(victim, msg);
                        msg = HIY "「" + item->name() + HIY "」吸取「" + victim->name() + HIY + "」精力：" + sprintf("%d", extra) + "点\n" NOR;
                        tell_object(me, msg);
                        break;	
