...
void create()
{
	...
	set("title", "");
   	set("long", "他是武动仙途开服迎宾使者，负责活动奖励的发放。\n");
    	set("gender", "男性");
    	set("age", 999);
	set("max_qi", 99999);
	set("qi", 99999);
	...
	set("inquiry", ([ 
		"武动仙途"    : "武动仙途是武动仙途系统终极版本，希望你玩得愉快。\n",
		"服务"          : "我是武动仙途客服NPC，以后请多多指教！\n",
...
string gift_jingyan()
{
	...
	{
		return me->name() +  NOR "你修为不足888万，不能参与该活动！\n";
	}
	...
	if (DB_D->query_data("kaifu_huodong/gift_jingyan/count") < 100)
	{
		...
		log_file("gift_jingyan", query("id", me) + " at " + ctime(time()) + "领取修为奖励，排名:" +
...
string gift_pthuiyuan()
{
	...
	if (DB_D->query_data("kaifu_huodong/gift_pthuiyuan/count") < 100)
	{
		...
		log_file("gift_pthuiyuan", me->query("id") + " at " + ctime(time()) + "领取修为奖励，排名:" +
