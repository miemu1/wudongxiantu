...
void do_kill(object target, object me)
{
        ...
        for(int i = a_amount; i > 0; i--) {     
                ...
                CHANNEL_D->channel_broadcast(CHANL_NAME, NOR + query("name", me) + NOR + "从" + query("name", target) + NOR + "的尸体上获得了" + 
                        ...
                        "潜能:" + spot + "，" + 
                        "体会:" + smar + "。\n" + NOR);
                log_file("static/yangyong", sprintf("  -修为%d、潜能%d、体会%d。\n",  sexp, spot, smar));
                sexp /= 2;
                spot /= 2;
                smar /= 2;
        }
        ...
        switch(step)
        {
                case 0:
                                message_vision("$N大声道：掷骰子现在开始。\n", this_object());
                                                        ...
                                                        spot /= sper;
                                                        smar /= sper;
                                                                        if (DEBUG) CHANNEL_D->channel_broadcast("wiz", NOR + "奖励比例:1/" + sper + "  修为:" + sexp +  "  潜能:" + spot +  "  体会:" + smar + "\n" + NOR);
                                                                }
                                break;
                case 1:
...
int ask_nt()
{
        ...
        message_vision(YEL + "$N" + NOR + YEL+ "从" + query("name") + NOR + YEL+ "手里接过一张百元武动仙途币。\n" + NOR, me);        
        log_file("static/yangyong3",sprintf("%s %s 100 $NT -> 百元武动仙途币, 账户余 %d $NT\n",TIME_D->replace_ctime(time()), query("id", me), amount));
...
LONG);
        ...
        set("inquiry", ([
                ...
                "兑换说明" : "如果member账户上有102 $NT，可以<ask yang yong about NT>兑换成一张百元武动仙途币。\n"+NOR,
