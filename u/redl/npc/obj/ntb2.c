...
void create()
{
        ...
        set_name(NOR BBLU HIW "武动仙途周岁股票" NOR, ({ "ntb" }));
        //基础价值25nt，修为商店出售为50nt(考虑最低七折情况下售价35nt)，一年后等于25+36=61nt
                set("base_value", d+25);
                set("long", NOR + WHT "这是发行在武动仙途周岁时的股票，目前可以(exchange)兑换成" + (string)(query("base_value")) + "点武动仙途币。\n" NOR);
                set("base_unit", "张");
...
int do_exchange()
{
        ...
        if(me->is_busy()) return notify_fail("你正忙着，没空兑换武动仙途币。\n");
        me->start_busy(2);
...
//            return notify_fail("没有这个人！\n");
 ...
 if (d > 50) d = 50;
        ...
        log_file("static/yangyong2",sprintf("%s %s 武动仙途周岁股->? $NT, 账户余 %d $NT\n",TIME_D->replace_ctime(time()), target, MEMBER_D->db_query_member(me, "money"))); 


        CHANNEL_D->channel_broadcast("chat", ( NOR + HIC + query("name", me) + NOR + HIC + "兑换武动仙途周岁股获得" + chinese_number(d) + NOR + HIC + "点武动仙途币。\n" + NOR )); 
//      write("兑换ok\n");
