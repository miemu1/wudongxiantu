...
void create()
{
	set_name(HIY "五绝铁令" NOR, ({ "wujue tieling" }));
	if (clonep())
		set_default_object(__FILE__);
	else {
		set("long", "凭此令可以学习一项其门派的武学，与五绝令牌不冲突。\n");
		set("unit", "块");
        set("no_give",1);
        set("no_drop",1);
        set("no_get",1);
...
int do_taojiao(string arg)
{
	...
	{
		tell_object(me,"修为不足60万，不能使用五绝铁令！\n");
		return 1;
	}
	...
	log_ufile(me,"wujueling","使用五绝令牌掌握了180级" + to_chinese(skill) + "。\n");
