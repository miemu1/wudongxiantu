...
void init_quest(string qob_name, int amount)
{
	...
	CHANNEL_D->do_channel(find_object(QUEST_D),"chat", place2 +"的"+npc2->name() +"有货物要送给"+ place1 +"的"+ npc1->name() +"。");
	// 设置对话信息
	npc1->set("inquiry", ([
		name      : "啊？你是送它来的？快给我。",
	...
	npc2->set("inquiry", ([
		...
		NPC1_NAME : "这批货就是要送给他的呀！他现在在"+place1+"附近！",
		NPC2_NAME : "正是区区，有何贵干？" ]));
...
mixed ask_for_qob(object npc2, object qob, string qob_name)
{
	...
	message_sort("$N看了看$n，说道：“这" + qob_name +
		     "您就交给我帮您运送吧，咱包您准时送"
		     "到！”\n", me, npc2);
	tell_object(me, CYN "你让" + npc2->name() +
			CYN "将" + qob_name +
			CYN "交托给你运送。"NOR"\n");

	if (me->query("score") < 1000)
	{
		message_sort("$N眯着眼睛瞥了$n一眼，哼了一声，"
			     "道：“" + RANK_D->query_respect(me) + 
			     "，你的江湖阅历太低，这批货给你我不放心。”\n",
			     npc2, me);
		return 1;
	}
	...
	if (objectp(qob) && environment(qob) == npc2 && qob->query_amount())
	{
		...
		{
			message_sort("$N眯着眼睛瞥了$n一眼，叹了口气，道：“" + RANK_D->query_respect(me) + 
				     "，这批货你运得了么？”\n",
				       npc2, me);
		}
	} else
		message_sort("$N微笑着对$n说：“不用劳烦你了，这批" + qob_name +
			     "已经有人送去了！”\n", npc2, me);
...
int npc_accept_object(object me, object who, object ob)
{
	...
	{
		message_sort("$N瞪着$n，使劲的摇着头道：“我要这干什么？”\n",
			     me, who);
		return 0;
	}
	...
	{
		// 所有的物品全部接收完了，准备结束任务
		message_vision("$N抹了抹头上的汗，道：“总算全运到了，这事可真够罗嗦的。”\n", me);
...
private void do_bonus(string who_id, object who)
{
	...
	log_file("deliver", ctime(time())+":"+who->query("name")+"("+who->query("id")+")"+"完成一次送货任务。\n");
	QUEST_D->bonus(who, b);
}
