	...
	if (clonep())
		set_default_object(__FILE__);
                ...
                set("long", HIG "一根晶莹剔透的权杖，好像是丐帮的玉竹杖哦！\n"
                                ...
                                "帮主通过它发号司令，邀请(author)他人入帮或是开除(expell)不听话的人。"NOR"\n");
                set("value", 1);
                set("no_sell", "这是什么垃圾？\n");
        	set("unit", "根");
                set("wield_msg", HIG "$N" HIG "亮出一个晶莹剔透的小棍棍，神气"
                                 "活现的喝道：“还不拜见...本...帮主？”"NOR"\n");
                set("unwield_msg", HIG "$N" HIG "鬼鬼祟祟的把权杖收好。"NOR"\n");
        }
...
int do_author(string arg)
{
        ...
        if (ob->query("combat_exp") >= 50000)
                return notify_fail("此人修为太高，不宜进入" + PARTY + "。\n");
	...
	me->set_temp("pending/author", ob);
        tell_object(ob, YEL + me->name(1) + "想收你入" + PARTY + "，你同意不同意(right/refuse)？"NOR"\n");
        return 1;
}
