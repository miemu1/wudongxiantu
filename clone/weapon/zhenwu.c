...
string do_wield()
{
	...
	if (me->query("shen") > 10000)
		return HIC "$N一声长啸，将$n" HIC "荡出剑鞘，霎时天地间浩气凛然。"NOR"\n";
	else
	if (me->query("shen") >= 0)
		return HIC "$N一伸手，已然把$n" HIC "亮在手中。"NOR"\n";
	else
	if (me->query("shen") > -10000)
		return HIG "$N战战兢兢的摸出一把$n" HIG "。"NOR"\n";
	else
		return HIG "$N一声冷笑，“唰”的一声亮出了$n" HIG "。"NOR"\n";
}
...
string do_unwield()
{
	...
	remove_call_out("check_npc");
	if (me->query("shen") >= 0)
		return HIC "$N一挥手，还剑入鞘。"NOR"\n";
	else
	if (me->query("shen") > -10000)
		return HIG "$N把$n" HIG "插回剑鞘，抹了抹头上的汗。"NOR"\n";
	else
		return HIG "$N一晒，把$n" HIG "插回剑鞘。"NOR"\n";
}
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if (me->query_skill_mapped("sword") != "taiji-jian" || me->query_skill("taiji-jian", 1) < 100)
		return damage_bonus / 2;
	...
	case 0:
		if (! victim->is_busy())
			victim->start_busy(me->query_skill("sword") / 20 + 2);
	...
	case 1:
		...
		return random(2) ? HIY "$N一声长吟，手中的真武剑化作一道长虹，“唰”的扫过$n。"NOR"\n":
				   HIY "$N突然大声喝道：“邪魔外道，还不受死？”手中真武剑"
				   HIY "忽的一抖，$n登时觉得眼前一花。"NOR"\n";
	}
