...
void create()
{
	set_name(HIR"凶兽·饕餮"NOR, ({"shouss taotie"}));
	set("nickname", "BOSS");
	set("gender", "无性");
	set("age", 9999);
	set("nolook", 1);
	...
	set_temp("apply/unarmed_damage", 8500);
	set_temp("apply/armor", 2000);

    setup();
    carry_object("/HELL6/GuaiWu3/Gzb/wuqi")->wield();
...
//	CHANNEL_D->do_channel(this_object(), "chat", HIR"唉，老身去也！"NOR"");
		...
		me->add("yuanbao",50);
        obj1 = new("/HELL6/BOX/box3");
        obj1->move(env);
		tell_object(me,sprintf(GRN"你获得了%d点修为，%d点潜能，%d点元神修为，50枚灵石，1个饕餮宝箱，请及时拾取，以免丢失\n"NOR,exp,pot,50000));
