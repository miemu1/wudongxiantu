...
LONG
   ...
   set("exits", ([ 
      "northeast" : "/quest/tulong/room/lake1",
   ]));
...
void boat_come(object me)
{
   ...
   if (!objectp(boat=find_object("/quest/tulong/room/boat")))
   ...
   boat->set("exits/up","/quest/tulong/room/lake2");
   set("exits/down","/quest/tulong/room/boat");
...
int valid_leave(object me, string dir)
{
   ...
   if (userp(me) && dir == "down") {
      boat=find_object("/quest/tulong/room/boat");
      if (!objectp(boat))
         boat=load_object("/quest/tulong/room/boat");
         inv=all_inventory(boat);
            for(i=0; i<sizeof(inv); i++) {
               ob = inv[i];
               if(living(ob)){
