...
*/
	...
	if (arg=="xianshi")
	{
		...
		msg= ZJOBLONG+HIR "◎"HIY "老魔扫荡系统" HIR "◎"NOR+ZJBR;
  ...
  //  拍卖物品：paimai <物品ID> for <底价值> <货币单位>
   // 竞拍物品：paimai <物品ID> add <加价值> <货币单位>
	...
	if(sscanf(arg,"%s for %d %s",str,num,unit) == 3)
	{
		...
		//{
		//	tell_object(me,"你虽已成年，但是涉世修为太少，无法对自己的拍卖行为负责。\n");
		//	return 1;
		//}
	...
	if(sscanf(arg,"%s add %d %s", ob_id,num,unit) == 3)
	{
		...
		if( num > 5000 )
		{
			write("现在限制一次最多家五千两黄金以避免不必要的问题产生\n");
			return 1;
		}
...
{    
    ...
    1. 玩家在18岁以后，自身修为值达到1000点即有权利参与拍卖。  
