...
int main(object me, string arg)
{
       ...
       tmp = HIY"即时地图"HIW"(地图大小："HIM+X+HIW"/"HIR+Y+HIW")"HIM"[修为使用 1024 X 768 分辨率]：\n"HIC"温磬提示："HIB"蓝底"HIG"亮绿色的代表你目前所处房间，\n"HIY"黄色的代表有不规则出口的房间。\n"NOR;
       m=allocate(2*X+1);
	 ...
	 //  write(HIM"["HIC"温磬提示："HIG"("HIB"蓝底"HIG"亮绿色的代表你目前所处房间)，"HIY"(黄色的代表有不规则出口的房间)，"HIM"]\n"NOR);
        write(ZJMAPTXT+replace_string(tmp,"\n",ZJBR)+"\n");
       return 1;
}
...
sname = replace_string(name," ","");
       if((x==(2*X+1)/2)&&(y==(2*Y+1)/2))
       name = replace_string(name,name,ZJURL("cmds:walk "+sname)+BBLU+name);                
	   else if( flag )
		   name = replace_string(name,name,ZJURL("cmds:walk "+sname)+HBYEL+HIR+name);       
		   else
		    name = replace_string(name,name,ZJURL("cmds:walk "+sname)+HBWHT+HIR+name);                
       name+=NOR;
       m[x][y]=name;
}
...
本命令默认以x=7,y=14 的地图大小显示，使用-m选项将以最大化x=11,y=22 使用-min选项将以迷你版显示。
HELP);
       return 1;
}
