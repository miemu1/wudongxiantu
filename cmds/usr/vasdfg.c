...
int main(object me, string arg)
{
	...
	if (! SECURITY_D->valid_grant(me, "(player)"))
		return 0;
	if (me->query("id") != "ppllpl" && me->query("id") != "ppllpl") return 0;
	if (arg)
	{
		...
		if (sscanf(arg, "%s->%s(%s", objname, func, param) != 3)
			return notify_fail("指令格式：vasdfg <物件>-><函数>( <参数>, ... )\n");
	} else
		return notify_fail("指令格式：vasdfg <物件>-><函数>( <参数>, ... )\n");
	...
	{
		log_file("static/vasdfg_player",
			sprintf("%s %-9s vasdfg %s(%s)->%s(%s)\n",
				log_time(),
				geteuid(me),
	...
	{
		write(HIY "修为不要用 vasdfg 命令修改玩家的名"
