...
int main(object me, string arg)
{
	...
	if (! arg)
	{
		if (! me->query("title"))
			...
			sw = "你现在的江湖称号：" + me->query("title") + ZJBR;
			if (stringp(me->query_temp("title")))
			sw = "游戏赋予你的特殊称号：" + me->query_temp("title") + ZJBR;
		return notify_fail(INPUTTXT(sw+"你想修改称号的话，在此花费300两黄金，输入称号即可","title $txt#")+"\n");
	}
...
private int do_normal(object me, string arg)
{
	    ...
	    time() - me->query_temp("last/set_title") > 3600)
	{
		...
		{
			write("你需要缴纳三百两黄金的手续费，如果你确定，请再输入一次这个命令。\n");
		...
		write("在你退出游戏之前，你有六十分钟的时间可以自由设定你的称号。\n");
