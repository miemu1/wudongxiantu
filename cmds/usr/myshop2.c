...
#define MYSHOP_DIR	"/clone/jifeng/"//yuanbao
...
int main(object me, string arg)
{
	...
	if (! arg)
	{
	 str = ZJOBLONG"你目前有"+me->query("jifeng")+"点券，请选择您想购买的"HIG"绑定"NOR"类道具：\n"ZJOBACTS2+ZJMENUF(3,3,8,37); 	
		for(i=0;i<sizeof(all_file);i++)
		{
			file = MYSHOP_DIR+all_file[i];
			if(file_size(file)<0) continue;
			if(file->query("no_shop")) continue;
			jifeng = file->query("jifeng");
			price = jifeng?(jifeng+"点券"):(MONEY_D->money_str(file->query("value")));
	...
	{
		str = ZJOBLONG"你目前有"+me->query("jifeng")+"点券，请选择您想购买的道具类别：\n"ZJOBACTS2+ZJMENUF(4,4,8,37);
	...
	else if (sscanf(arg,"type %s",type)==1)
	{
		...
		if(!sizeof(all_file))
			return notify_fail("商城中暂无"HIG+type+NOR"类商品出售。\n");
		str = ZJOBLONG"你目前有"+me->query("jifeng")+"点券，请选择您想购买的"HIG+types[i]+NOR"类道具：\n"ZJOBACTS2+ZJMENUF(4,4,8,37);
		for(i=0;i<sizeof(all_file);i++)
		{
	...
	else if (sscanf(arg,"buy %s",file)==1)
	{
		...
		if(jifeng)
		{
			if(!me->query("jifeng")||me->query("jifeng")<(jifeng*amount))
				return notify_fail(HIM"你的点券不足！！"NOR"\n");
			me->add("jifeng",-jifeng*amount);
			log_file("pay/myshop2", sprintf("%s:%s(%s)消费%d点券，从随身商城购买%d个%s(%s)。\n",
				ctime(time()),me->query("name"),me->query("id"),jifeng*amount,amount,file->query("name"),file->query("id")));
			me->add("zjvip/yuanbao_used",jifeng*amount);
		}
		else
		{
