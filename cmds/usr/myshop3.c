...
int main(object me, string arg)
{
	...
	if(!sizeof(all_file))
		return notify_fail("商城中暂无"HIG"特价"NOR"类商品出售\n");

	if (! arg)
	{
	 str = ZJOBLONG"你目前有"+me->query("yuanbao")+"灵石，请选择您想购买的"HIG"特价"NOR"类道具："ZJBR+HIR"特价道具均只可使用一次，均为绑定商品"NOR"。\n"ZJOBACTS2+ZJMENUF(3,3,8,37); 	
	...
	{
		str = ZJOBLONG"你目前有"+me->query("yuanbao")+"灵石，请选择您想购买的道具类别："ZJBR+HIR"特价道具均只可使用一次，均为绑定商品"NOR"。\n"ZJOBACTS2+ZJMENUF(4,4,8,37);
	...
	else if (sscanf(arg,"look %s",file)==1)
	{
		...
		str += "购买1个:myshop3 buy 1 "+file+ZJSEP"批量购买:myshop3 buy "+file+"\n";
		write(str+"\n");
		return 1;
	}
	else if (sscanf(arg,"buy %s",file)==1)
	{
		if (sscanf(file, "%d %s", amount, file) != 2)
		{
			...
			write(INPUTTXT("你想买多少个"+file->query("name")+"？"ZJBR"请确认你的背包空间足够。","myshop3 buy $txt# "+file)+"\n");
			return 1;
		}
		...
		if(yuanbao)
		{
			if(!me->query("yuanbao")||me->query("yuanbao")<(yuanbao*amount))
				return notify_fail(HIM"你的灵石不足！！"NOR"\n");
			me->add("yuanbao",-yuanbao*amount);
			log_file("pay/myshop3", sprintf("%s:%s(%s)消费%d灵石，从随身商城购买%d个%s(%s)。\n",
		...
		else
		{
			...
			log_file("pay/myshop3", sprintf("%s:%s(%s)消费%s，从随身商城购买%d个%s。\n",
