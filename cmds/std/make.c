...
int making(object me)
{
	...
	case 9:
			   ...
			   qm->query("item_prop/medical")) >= req["min_level"])
		{
			ob = new(med);
			ob->move(qm, 1);
			write(HIW"你的炼药熟练度增加100！\n");
			msg_me = HIM "你把「" + ob->name() + HIM "」成功的制好了！"NOR"\n";
			me->improve_skill("medical",100);
			msg_out = HIM "$N" HIM "嘴角露出一丝微笑。"NOR"\n";
			
			
		} else if(!me->query("special_skill/lucky") || random(2) == 0)
		{
			msg_me = HIG "真令人沮丧！居然失败了，真是可惜了。"NOR"\n";
			write(HIW"你的炼药熟练度增加15！\n");
			me->improve_skill("medical",15);
...
{
	message_vision("$N把手中的研钵一翻个，东西全都倒了出来。\n", me);
	stop_making(me);
	return 1;
}
