...
int main(object me, string arg)
{
	...
	if (target->query_competitor())
		return notify_fail("人家正在和别人比武，你要是暗算人家也未免太无耻了吧？\n");
	...
	if (! target->query("can_speak"))
		return notify_fail("你这人真有意思，连" + target->name() + "也想暗算。\n");
	...
	write( CYN"你摒息静气，纵身而起，向" + dir + "的" + target->name() + "扑去！"NOR"\n");
	...
	if (random(me->query("combat_exp")) > (int)target->query("combat_exp") / 2 &&
	    ! target->is_guarder())
	{
		message_vision(CYN"$N一时没有防范，被$n攻了个措手不及！"NOR"\n", target, me);
		if (! target->is_busy())
			target->start_busy(random(4) + 2);
	} else
	{
		message_vision(CYN "$N修为丰富，不及细看，立刻向$n反击！"NOR"\n", target, me);
	    ...
	    target->is_guarder())
	{
		...
		tell_object(target, CYN "你一定神，原来是" + me->name() + "在暗算我！"NOR"\n");
		write(CYN "不好，被" + target->name() + "发觉了！"NOR"\n");
		message_vision(CYN "$N喝道：$n！你敢暗算我，我跟你没完！"NOR"\n", target, me);
	...
	{
		message("vision", CYN + gen + "转瞬不见！"NOR"\n",environment(target), me);
		tell_object(me, CYN"你立刻退回原处！"NOR"\n");
		...
		tell_object(target, CYN "你一定神，可是什么都没看见。"NOR"\n");
		message("vision", CYN + target->name() + "一脸惶恐，竟不知道谁在暗算他！"NOR"\n",
				  environment(target), target);
		write(CYN "哈哈，" + target->name() + "居然没看出来。"NOR"\n");
		write(CYN "你搓了搓手，一脸无辜的样子。"NOR"\n");
	}
