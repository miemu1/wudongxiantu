...
{
	string tmpstr,from, item, where, tmp;
	...
	if (sscanf(arg, "%d %s", amount, item) == 2)
	{
		...
		if (obj->query_temp("no_auto_get") && sscanf(item,"/%*s/%s",tmpstr)!=2)
		{
			me->start_busy(1);
			return notify_fail("此物品只能点击拾取。\n");
		}
	...
	if (obj->query_temp("no_auto_get") && sscanf(arg,"/%*s/%s",tmpstr)!=2)
	{
		me->start_busy(1);
		return notify_fail("此物品只能点击拾取。\n");
	}
...
int do_get(object me, object obj, int raw)
{
	...
	//if(playerp(obj))
	if (obj->is_character() && obj->query("race") == "人类")
		return notify_fail("本游戏不支持把活人塞进背包。\n");

	if (obj->query_amount()) num = obj->query_amount();
	log_file("action/get", sprintf("%s，%s(%s)捡起%s。\n",ctime(time()),me->query("name"),me->query("id"),obj->short()));
	if (obj->move(me))
	{
		obj->delete_temp("no_auto_get");
		...
		} else
		{
			msg = sprintf("$N%s%s%s%s。\n",
				...
				old_env->is_tree() ? "从" + old_env->name() +"上摘下" : "从" + old_env->name() + "中拿出",
				obj->query_amount()?CHINESE_D->chinese_number(num):"一",
