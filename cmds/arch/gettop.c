...
void search_dir(object me, int type)
{
	...
	log_file("gettop", "SYSTEM start gettop all player on " + ctime(time()) + ".\n");
	...
	if (result)
	{
		if (online)
			result = sprintf("%s%-14s%-10s%s%-17s%s%s\n",
				 ...
				 (interactive(user_ob) ? query_ip_number(user_ob) : "神游中"),
				 NOR,
				 result);
		else
...
private string is_illegal(object ob, int update)
{
	...
	if (exp < 1000000 && gold >= 1000)
		return HIC "修为 1M 以内拥有 " + gold + " 两黄金" NOR;

	if (exp > 10000 && dt > 60)
	{
		if ((sp = dexp * 60 / dt) > 90)
			return HIR "平均修为速度太快(" + sp + "/分钟:" + dt / 60 + "分钟)" NOR;
