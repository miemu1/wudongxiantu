...
int main(object me, string arg)
{
	...
	if (arg == "?")
	{
		...
		if (mapp(log) && sizeof(log) > 0)
		{
			...
			for (i = 0; i < sizeof(ks); i++)
			{
				...
				msg += sprintf("%s%-14s  目前被  %-14s 记录日志。"NOR"\n",
		...
		for (i = 0; i < sizeof(obs); i++)
		{
			...
			msg += sprintf("%s%-20s  " HIC "%-10s  %s"NOR"\n",
	...
	if (arrayp(grant = SECURITY_D->query_grant(arg)) &&
	    member_array("no_log", grant) != -1)
		/* Do not log */
		return notify_fail("你不能记录这个玩家的日志。\n");
