...
int main(object me, string arg)
{
	...
	if (! (ob = find_player(user)))
	{
		...
		if (objectp(ob = find_player(user)))
		{
			if (clear_flag)
			{
				tell_object(me, HIG "清除了玩家" + ob->name(1) + "的某些相关数据。"NOR"\n");
				clear_user_data(ob);
			}
		} else
			tell_object(me, HIG "无法载入该玩家。"NOR"\n");
		return 1;
	}

	// restore the object
	tell_object(ob, HIM "\n从磁盘中载入你的数据，并重新设置。"NOR"\n");
	if (! ob->restore())
	{
	...
	foreach (item in all_inventory(ob))
	{
		...
		{
			tell_object(me, item->name(1) + "从玩家" +
					ob->name(1) + "身上脱离。\n");
	...
	// force look
	tell_object(ob, HIY  "> l"NOR"\n");
	catch(ob->force_me("look"));

	// force hp
	tell_object(ob, HIY  "> hp"NOR"\n");
	catch(ob->force_me("hp"));

	// force show
	ob->write_prompt();
	tell_object(ob, HIY "check backup data now\n" +
			HIC "请检查你目前的状况是否正确。"NOR"\n");
	if (ob != me)
		tell_object(me, HIC "\n玩家已经重新设置。"NOR"\n");

	if (clear_flag)
	{
		write(HIG "清除了玩家" + ob->name(1) + "的某些相关数据。"NOR"\n");
		clear_user_data(ob);
	}
	return 1;
}
...
免数据不一致。如果玩家受到损失需要恢复，则应察看恢复前是否有
道具方面的损失，如果全部损失了，那么可以用 -c 参数恢复，自动
清除所有相关数据。如果没有损失，可以直接恢复。如果损失部分，
可以直接恢复然后使用 clear  命令清除那些和损失相关的数据。
