...
int closing(object me)
{
	...
	if (pot <= me->query("learned_points"))
	{
		...
		CHANNEL_D->do_channel(this_object(), "rumor",sprintf("听说%s(%s)闭关功德圆满。",me->name(1), me->query("id")));
		if (! interactive(me))
		{
	...
	{
		tell_object(me, HIR "你对内功有所领悟，感到内力进步了！"NOR"\n");
	...
	{
		tell_object(me, HIM "你对神通有所领悟，感到精力进步了！"NOR"\n");
	...
	{
		r = random(r);
		tell_object(me, HIY "你对" + to_chinese(ks[r]) + "有所感悟。"NOR"\n");
...
int halt_closing(object me)
{
	...
	message_vision(HIY "$N" HIY "大喝一声，睁开眼来，一股气流登时将众人迫退四步。"NOR"\n", me);
...
(halt)或是潜能耗尽。期间内玩家的修为、技能、内力将会提升。
