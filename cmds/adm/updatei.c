...
int main(object me, string arg)
{
	...
	obs = filter_array(objects(), (: inherits($(file), $1) && ! userp($1) && ! playerp($1) :));
	obs -= ({ find_object(VOID_OB), find_object(TEMP_OB) });
	if (sizeof(obs) > 1024 && ! opt_force)
	{
		write("一共有 " + sizeof(obs) + " 个物件牵涉在内，如果你决定编译，请指明-f参数。\n");
	}
	...
	if (count > 0)
		write(HIC "总共有" + HIW + count + HIC "个档案" +
		      (opt_compile ? "被成功编译！"NOR"\n" : "需要编译。"NOR"\n"));
	else
		write(HIC "没有编译任何档案需要编译。"NOR"\n");
