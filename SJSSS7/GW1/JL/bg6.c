...
//读入数据
	...
	set("unit", "件");
		set("long", HIC"【合体限定】"NOR"合体系列								
"HIG"《套装加成：修仙真法等级+400，攻击力70000》"NOR"\n");
	set("value",1000);
	set("armor_prop/armor", 7000);
	set("armor_prop/leech_qi", 1);
	set("armor_prop/reduce_poison", 1);
	set("armor_prop/reduce_magic", 1);
	set("armor_prop/unarmed_damage", 7000+lv1+lv2+lv3);
	set("armor_prop/damage", 7000+lv1+lv2+lv3);	
	set("no_get",1);
	set("no_drop",1);
	set("no_put",1);
	...
	set("armor_type","head");
		set("material", "fur");
				 set("suit",HIR"合体套装"NOR);  //套装名称
		set("suit_lvl",6);	     //套装等级
		set("suit",HIR"合体套装"NOR);  //套装名称
		set("suit_lvl",6);	     //套装等级
		set("suit_count",9);	   //套装部件数量
	      //套装各部件
		set("suit_mod/arrmor","合体·护甲");
		set("suit_mod/finger","合体·戒指");
		set("suit_mod/head","合体·头饰");
		set("suit_mod/neck","合体·项链");
		set("suit_mod/pants","合体·护腿");
		set("suit_mod/boots","合体·护靴");
		set("suit_mod/cloth","合体·衣服");
		set("suit_mod/waist","合体·腰带");
		set("suit_mod/wrists","合体·护腕");
	    //套装全部附加天赋效果  //身法第一，根骨第二，悟性第三，臂力第四
		set("suit_eff/constitution",70);      //3件
		set("suit_eff/spirituality",70);      // 5件
		set("suit_eff/intelligence",70);      //7件
		set("suit_eff/strength",70);      //9件
	       //套装全部附加技能效果，一整套
        set("suit_eff_skill/xiuxian-gongfa",400+me->query("taozhuang/lv"));
	setup();
}
